async function loadData() {
    try {
      ...
      for (const [semester, courses] of Object.entries(data)) {
        if (semester.startsWith("نیمسال") || semester.startsWith("درس های")) {
          ...
          courses.forEach((course) => {
            htmlContent += `<tr${course.وضعیت ? ' class="highlight"' : ""}>
                          <td>${course.ردیف}</td>
      ...
      checkboxes.forEach((checkbox) => {
        ...
        if (checkbox.checked) {
          updateUnitSummary(type, unit, true);
        }
      });
  ...
  function updateRemainingCourses() {
    const remainingCoursesTable = document.querySelector(
      "#remaining-courses tbody"
    );
